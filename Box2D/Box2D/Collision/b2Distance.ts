...
//    }
  ...
  public GetSupport(d: b2Vec2): number {
    ...
    let bestValue: number = b2DotVV(this.m_vertices[0], d);
    for (let i: number = 1; i < this.m_count; ++i) {
      const value: number = b2DotVV(this.m_vertices[i], d);
      if (value > bestValue) {
        bestIndex = i;
        bestValue = value;
      }
...
export class b2Simplex {
  ...
  public GetSearchDirection(out: b2Vec2): b2Vec2 {
    ...
    case 2: {
        const e12: b2Vec2 = b2SubVV(this.m_v2.w, this.m_v1.w, out);
        const sgn: number = b2CrossVV(e12, b2NegV(this.m_v1.w, b2Vec2.s_t0));
        if (sgn > 0) {
  ...
  public Solve2(): void {
    ...
    const e12: b2Vec2 = b2SubVV(w2, w1, b2Simplex.s_e12);

    // w1 region
    const d12_2: number = (-b2DotVV(w1, e12));
    if (d12_2 <= 0) {
    ...
    // w2 region
    const d12_1: number = b2DotVV(w2, e12);
    if (d12_1 <= 0) {
  ...
  public Solve3(): void {
    ...
    // a3 = 0
    const e12: b2Vec2 = b2SubVV(w2, w1, b2Simplex.s_e12);
    const w1e12: number = b2DotVV(w1, e12);
    const w2e12: number = b2DotVV(w2, e12);
    ...
    // [w1.e13 w3.e13][a3] = [0]
    // a2 = 0
    const e13: b2Vec2 = b2SubVV(w3, w1, b2Simplex.s_e13);
    const w1e13: number = b2DotVV(w1, e13);
    const w3e13: number = b2DotVV(w3, e13);
    ...
    // [w2.e23 w3.e23][a3] = [0]
    // a1 = 0
    const e23: b2Vec2 = b2SubVV(w3, w2, b2Simplex.s_e23);
    const w2e23: number = b2DotVV(w2, e23);
    const w3e23: number = b2DotVV(w3, e23);
    const d23_1: number = w3e23;
    const d23_2: number = (-w2e23);

    // Triangle123
    const n123: number = b2CrossVV(e12, e13);

    const d123_1: number = n123 * b2CrossVV(w2, w3);
    const d123_2: number = n123 * b2CrossVV(w3, w1);
    const d123_3: number = n123 * b2CrossVV(w1, w2);
...
export function b2Distance(output: b2DistanceOutput, cache: b2SimplexCache, input: b2DistanceInput): void {
  ...
  if (input.useRadii) {
    ...
    if (output.distance > (rA + rB) && output.distance > b2_epsilon) {
      ...
      const normal: b2Vec2 = b2SubVV(output.pointB, output.pointA, b2Distance_s_normal);
      normal.Normalize();
      ...
      const p: b2Vec2 = b2MidVV(output.pointA, output.pointB, b2Distance_s_p);
