...
export function b2CollideEdgeAndCircle(manifold, edgeA, xfA, circleB, xfB) {
  ...
  const Q: b2Math.b2Vec2 = b2Math.b2Transform.MulTXV(xfA, b2Math.b2Transform.MulXV(xfB, circleB.m_p, b2Math.b2Vec2.s_t0), b2CollideEdgeAndCircle_s_Q);

  const A: b2Math.b2Vec2 = edgeA.m_vertex1;
  const B: b2Math.b2Vec2 = edgeA.m_vertex2;
  const e: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(B, A, b2CollideEdgeAndCircle_s_e);

  // Barycentric coordinates
  const u: number = b2Math.b2Vec2.DotVV(e, b2Math.b2Vec2.SubVV(B, Q, b2Math.b2Vec2.s_t0));
  const v: number = b2Math.b2Vec2.DotVV(e, b2Math.b2Vec2.SubVV(Q, A, b2Math.b2Vec2.s_t0));
  ...
  if (v <= 0) {
    ...
    const d: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(Q, P, b2CollideEdgeAndCircle_s_d);
    const dd: number = b2Math.b2Vec2.DotVV(d, d);
    if (dd > radius * radius) {
      return;
    }
    ...
    if (edgeA.m_hasVertex0) {
      ...
      const e1: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(B1, A1, b2CollideEdgeAndCircle_s_e1);
      const u1: number = b2Math.b2Vec2.DotVV(e1, b2Math.b2Vec2.SubVV(B1, Q, b2Math.b2Vec2.s_t0));
  ...
  if (u <= 0) {
    ...
    if (edgeA.m_hasVertex3) {
      ...
      const e2: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(B2, A2, b2CollideEdgeAndCircle_s_e2);
      const v2: number = b2Math.b2Vec2.DotVV(e2, b2Math.b2Vec2.SubVV(Q, A2, b2Math.b2Vec2.s_t0));
  ...
  // Region AB
  const den: number = b2Math.b2Vec2.DotVV(e, e);
...
class b2EPCollider {
  ...
  public Collide(manifold, edgeA, xfA, polygonB, xfB) {
    ...
    const edge1: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(this.m_v2, this.m_v1, b2EPCollider.s_edge1);
    edge1.Normalize();
    this.m_normal1.SetXY(edge1.y, -edge1.x);
    const offset1: number = b2Math.b2Vec2.DotVV(this.m_normal1, b2Math.b2Vec2.SubVV(this.m_centroidB, this.m_v1, b2Math.b2Vec2.s_t0));
    ...
    if (hasVertex0) {
      const edge0: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(this.m_v1, this.m_v0, b2EPCollider.s_edge0);
      edge0.Normalize();
    ...
    if (hasVertex3) {
      const edge2: b2Math.b2Vec2 = b2Math.b2Vec2.SubVV(this.m_v3, this.m_v2, b2EPCollider.s_edge2);
      edge2.Normalize();
    ...
    if (primaryAxis.type === b2EPAxisType.e_edgeA) {
      ...
      let bestValue: number = b2Math.b2Vec2.DotVV(this.m_normal, this.m_polygonB.normals[0]);
      for (let i: number = 1, ict: number = this.m_polygonB.count; i < ict; ++i) {
        const value: number = b2Math.b2Vec2.DotVV(this.m_normal, this.m_polygonB.normals[i]);
        if (value < bestValue) {
          bestValue = value;
          bestIndex = i;
        }
  ...
  public ComputeEdgeSeparation(out) {
    ...
    for (let i: number = 0, ict = this.m_polygonB.count; i < ict; ++i) {
      const s: number = b2Math.b2Vec2.DotVV(this.m_normal, b2Math.b2Vec2.SubVV(this.m_polygonB.vertices[i], this.m_v1, b2Math.b2Vec2.s_t0));
  ...
  public ComputePolygonSeparation(out) {
    ...
    for (let i: number = 0, ict = this.m_polygonB.count; i < ict; ++i) {
      const n: b2Math.b2Vec2 = b2Math.b2Vec2.NegV(this.m_polygonB.normals[i], b2EPCollider.s_n);

      const s1: number = b2Math.b2Vec2.DotVV(n, b2Math.b2Vec2.SubVV(this.m_polygonB.vertices[i], this.m_v1, b2Math.b2Vec2.s_t0));
      const s2: number = b2Math.b2Vec2.DotVV(n, b2Math.b2Vec2.SubVV(this.m_polygonB.vertices[i], this.m_v2, b2Math.b2Vec2.s_t0));
