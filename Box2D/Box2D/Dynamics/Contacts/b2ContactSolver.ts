...
export class b2ContactSolver {
  ...
  public InitializeVelocityConstraints() {
    ...
    let iA: number;
    let iB: number;
    let localCenterA: b2Vec2;
    let localCenterB: b2Vec2;
    ...
    for (i = 0, ict = this.m_count; i < ict; ++i) {
      ...
      for (j = 0, jct = pointCount; j < jct; ++j) {
        ...
        rnA = b2Vec2.CrossVV(vcp.rA, vc.normal);
        rnB = b2Vec2.CrossVV(vcp.rB, vc.normal);
        ...
        rtA = b2Vec2.CrossVV(vcp.rA, tangent);
        rtB = b2Vec2.CrossVV(vcp.rB, tangent);
      ...
      if (vc.pointCount === 2) {
        ...
        rn1A = b2Vec2.CrossVV(vcp1.rA, vc.normal);
        rn1B = b2Vec2.CrossVV(vcp1.rB, vc.normal);
        rn2A = b2Vec2.CrossVV(vcp2.rA, vc.normal);
        rn2B = b2Vec2.CrossVV(vcp2.rB, vc.normal);
  ...
  public SolveVelocityConstraints() {
    ...
    for (i = 0, ict = this.m_count; i < ict; ++i) {
      ...
      for (j = 0, jct = pointCount; j < jct; ++j) {
        ...
        vt = b2Vec2.DotVV(dv, tangent) - vc.tangentSpeed;
      ...
      if (vc.pointCount === 1) {
        ...
        vn1 = b2Vec2.DotVV(dv1, normal);
        // float32 vn2 = b2Dot(dv2, normal);
        vn2 = b2Vec2.DotVV(dv2, normal);
  ...
  public SolvePositionConstraints() {
    ...
    for (i = 0, ict = this.m_count; i < ict; ++i) {
      ...
      for (j = 0, jct = pointCount; j < jct; ++j) {
        ...
        rnA = b2Vec2.CrossVV(rA, normal);
        // float32 rnB = b2Cross(rB, normal);
        rnB = b2Vec2.CrossVV(rB, normal);
