...
export class b2RopeJoint extends b2Joint {
  ...
  public InitVelocityConstraints(data) {
    ...
    const crA: number = b2CrossVV(this.m_rA, this.m_u);
    const crB: number = b2CrossVV(this.m_rB, this.m_u);
    ...
    if (data.step.warmStarting) {
      ...
      const P: b2Vec2 = b2MulSV(this.m_impulse, this.m_u, b2RopeJoint.InitVelocityConstraints_s_P);
      // vA -= m_invMassA * P;
  ...
  public SolveVelocityConstraints(data) {
    ...
    const vpA: b2Vec2 = b2AddVCrossSV(vA, wA, this.m_rA, b2RopeJoint.SolveVelocityConstraints_s_vpA);
    // b2Vec2 vpB = vB + b2Cross(wB, m_rB);
    const vpB: b2Vec2 = b2AddVCrossSV(vB, wB, this.m_rB, b2RopeJoint.SolveVelocityConstraints_s_vpB);
    // float32 C = m_length - m_maxLength;
    const C: number = this.m_length - this.m_maxLength;
    // float32 Cdot = b2Dot(m_u, vpB - vpA);
    let Cdot: number = b2DotVV(this.m_u, b2SubVV(vpB, vpA, b2Vec2.s_t0));
    ...
    const P: b2Vec2 = b2MulSV(impulse, this.m_u, b2RopeJoint.SolveVelocityConstraints_s_P);
    // vA -= m_invMassA * P;
    vA.SelfMulSub(this.m_invMassA, P);
    wA -= this.m_invIA * b2CrossVV(this.m_rA, P);
    // vB += m_invMassB * P;
  ...
  public SolvePositionConstraints(data) {
    ...
    // b2Vec2 P = impulse * u;
    const P: b2Vec2 = b2MulSV(impulse, u, b2RopeJoint.SolvePositionConstraints_s_P);
  ...
  public GetReactionForce(inv_dt: number, out: b2Vec2): b2Vec2 {
    const F: b2Vec2 = b2MulSV((inv_dt * this.m_impulse), this.m_u, out);
    return F;
  ...
  public Dump(): void {
    if (DEBUG) {
      ...
      b2Log("  const jd: b2RopeJointDef = new b2RopeJointDef();\n");
