...
export class b2PrismaticJoint extends b2Joint {
  ...
  public InitVelocityConstraints(data) {
    ...
    if (this.m_enableLimit) {
      ...
      const jointTranslation: number = b2DotVV(this.m_axis, d);
  ...
  public SolveVelocityConstraints(data) {
    ...
    if (this.m_enableMotor && this.m_limitState !== b2LimitState.e_equalLimits) {
      ...
      const Cdot: number = b2DotVV(this.m_axis, b2SubVV(vB, vA, b2Vec2.s_t0)) + this.m_a2 * wB - this.m_a1 * wA;
      ...
      const P: b2Vec2 = b2MulSV(impulse, this.m_axis, b2PrismaticJoint.SolveVelocityConstraints_s_P);
    ...
    const Cdot1_x: number = b2DotVV(this.m_perp, b2SubVV(vB, vA, b2Vec2.s_t0)) + this.m_s2 * wB - this.m_s1 * wA;
    // Cdot1.y = wB - wA;
    const Cdot1_y = wB - wA;
    ...
    if (this.m_enableLimit && this.m_limitState !== b2LimitState.e_inactiveLimit) {
      ...
      const Cdot2: number = b2DotVV(this.m_axis, b2SubVV(vB, vA, b2Vec2.s_t0)) + this.m_a2 * wB - this.m_a1 * wA;
      ...
      const P: b2Vec2 = b2MulSV(df2.x, this.m_perp, b2PrismaticJoint.SolveVelocityConstraints_s_P);
  ...
  public SolvePositionConstraints(data) {
    ...
    const axis: b2Vec2 = b2MulRV(qA, this.m_localXAxisA, this.m_axis);
    // float32 a1 = b2Cross(d + rA, axis);
    const a1 = b2CrossVV(b2AddVV(d, rA, b2Vec2.s_t0), axis);
    // float32 a2 = b2Cross(rB, axis);
    const a2 = b2CrossVV(rB, axis);
    // b2Vec2 perp = b2Mul(qA, m_localYAxisA);
    const perp: b2Vec2 = b2MulRV(qA, this.m_localYAxisA, this.m_perp);

    // float32 s1 = b2Cross(d + rA, perp);
    const s1 = b2CrossVV(b2AddVV(d, rA, b2Vec2.s_t0), perp);
    // float32 s2 = b2Cross(rB, perp);
    const s2 = b2CrossVV(rB, perp);

    // b2Vec3 impulse;
    let impulse = b2PrismaticJoint.SolvePositionConstraints_s_impulse;
    // b2Vec2 C1;
    // C1.x = b2Dot(perp, d);
    const C1_x: number = b2DotVV(perp, d);
    ...
    if (this.m_enableLimit) {
      ...
      const translation: number = b2DotVV(axis, d);
      if (b2Abs(this.m_upperTranslation - this.m_lowerTranslation) < 2 * b2_linearSlop) {
  ...
  public GetJointTranslation(): number {
    ...
    // b2Vec2 d = pB - pA;
    const d: b2Vec2 = b2SubVV(pB, pA, b2PrismaticJoint.GetJointTranslation_s_d);
  ...
  public GetJointSpeed(): number {
    ...
    const rA: b2Vec2 = b2MulRV(bA.m_xf.q, this.m_lalcA, this.m_rA);
    // b2Vec2 rB = b2Mul(bB->m_xf.q, m_localAnchorB - bB->m_sweep.localCenter);
    b2SubVV(this.m_localAnchorB, bB.m_sweep.localCenter, this.m_lalcB);
    const rB: b2Vec2 = b2MulRV(bB.m_xf.q, this.m_lalcB, this.m_rB);
    // b2Vec2 pA = bA->m_sweep.c + rA;
    const pA: b2Vec2 = b2AddVV(bA.m_sweep.c, rA, b2Vec2.s_t0); // pA uses s_t0
    // b2Vec2 pB = bB->m_sweep.c + rB;
    const pB: b2Vec2 = b2AddVV(bB.m_sweep.c, rB, b2Vec2.s_t1); // pB uses s_t1
    // b2Vec2 d = pB - pA;
    const d: b2Vec2 = b2SubVV(pB, pA, b2Vec2.s_t2); // d uses s_t2
  ...
  public Dump() {
    if (DEBUG) {
      ...
      b2Log("  const jd: b2PrismaticJointDef = new b2PrismaticJointDef();\n");
