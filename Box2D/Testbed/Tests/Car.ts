...
export class Car extends Test {
  ...
  constructor(canvas: HTMLCanvasElement, settings: Settings) {
    ...
    let ground: b2Body = null;
    if (true) {
      const bd: b2BodyDef = new b2BodyDef();
      ground = this.m_world.CreateBody(bd);

      const edge_shape: b2EdgeShape = new b2EdgeShape();
    ...
    if (true) {
      const N: number = 20;
      const polygon_shape: b2PolygonShape = new b2PolygonShape();
    ...
    if (true) {
      ...
      const wjd: b2WheelJointDef = new b2WheelJointDef();
